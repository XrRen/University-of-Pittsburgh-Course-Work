...
public class BoggleSolver
{
	...
	private void solve(int row, int col, /*for debugging*/ int depth){
		...
		for(int direction=0; direction<8; direction++){
			if(isValid(row, col, direction)){
				...
				if(res == 1){ //prefix but not word, keep searching
					solve(nextCoords.row, nextCoords.col, depth + 1);
				}
				if(res == 2){ //word but not prefix, the result
	...
	// are the coordiates real
	// are the words been used before.

	private boolean isValid(int row, int col, int direction){
		Coordinates coords = nextCoordinates(row, col, direction);
		if(coords.row > 3 || coords.col > 3 || coords.col < 0 || coords.row < 0)
		{
			return false;
		}
		else if(Character.isUpperCase(theBoard[coords.row][coords.col]))
		{
			return false;
		}
		else
