  ...
  */
  ...
  private void readGraph() throws IOException {
    ...
    while(fileScan.hasNext()){
      ...
      FlowEdge edge = new FlowEdge(from-1, to-1, weight);
      G.addEdge(edge);
      //TODO: Add the same edge to the adjacency list of vetrex (to -1) 
      //to act as a placeholder for the backwards edge
      G.adj[to-1].add(edge);
      if(fileScan.hasNext())
        fileScan.nextLine();
    }
  ...
  private class Digraph {
    ...
    */
    ...
    public void bfs(int source) {
      ...
      while (!q.isEmpty()) {
        ...
        for (FlowEdge e : adj(v)) {
          //TODO: check if residual capacity from v to the 
          //other end point of e is larger 0
            if (e.residualCapacityTo(e.other(v)) > 0 && !marked[e.other(v)]) {
              edgeTo[e.other(v)] = e;
    ...
    private boolean hasAugmentingPath(int s, int t) {
      //TODO: start breadth-first search from s and return marked[t]
      bfs(s);
      return marked[t];
    }
